function myTraj = bmTraj_rotate_x_fullRadial3_phyllotaxis_lineAssym2(varargin)
...
%   containing the needed variables or the 6 variables separately:
...
% nShot
disp('Modified version for testing')
[theta, phi] = bmPhyllotaxisAngle3(nSeg, nShot, flagSelfNav);
%theta = theta(1:419*22);
%phi = phi(1:419*22);
...
x2 = reshape(R.*cos(phi).*sin(theta), [1, N_n, nSeg, nShot]);
y2 = reshape(R.*sin(phi).*sin(theta), [1, N_n, nSeg, nShot]);
