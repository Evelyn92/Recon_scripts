...
%%
reconpath = '/Users/cag/Documents/Dataset/recon_results/250127_recon/Sub001/figs';
...
%%
for slice_idx=230:260
    diff_map_plot(x_bin_crop, x_wobin_crop, slice_idx)
end
...
ax1 = subplot(3, 1, 1);
imshow(x1(:, :, slice_idx), [0,2]); axis off; title(strcat('bin-', num2str(slice_idx)));
colormap(ax1, gray); % Set colormap only for this subplot
colorbar;

ax2 = subplot(3, 1, 2);
% imagesc
imshow(x2(:, :, slice_idx), [0,2]); axis off;  title(strcat('wobin-', num2str(slice_idx)));
colormap(ax2, gray);
colorbar;

ax3 = subplot(3, 1, 3);
loLev = 0.1;upLev = 0.7;[imClip, rgb_vec] = relaxationColorMap('T1', diff_map(:,:,slice_idx), loLev, upLev);
